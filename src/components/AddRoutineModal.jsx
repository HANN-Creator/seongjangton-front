  ...
  return (
    <div className="fixed inset-0 bg-white flex flex-col">
      ...
      <header className="h-12 flex items-center justify-center border-b bg-white relative">
        <button onClick={onClose} className="absolute left-4 p-2 bg-gray-100 rounded-lg">
        ...
        <div className="w-28 h-28 rounded-full border-2 border-green-200 bg-green-50 flex items-center justify-center mb-4">
          <div className="w-14 h-14 rounded-full border border-gray-300 bg-white flex items-center justify-center">
            <img
              src={
                phase === 'icon'
                  ? (selectedIcon ? ICON_OPTIONS.find(i => i.id === selectedIcon).src : CheckIcon)
        ...
        {phase === 'icon' && (
          <div className="grid grid-cols-4 gap-x-6 gap-y-4 mt-auto mb-1">
            {ICON_OPTIONS.map(opt => (
              ...
              >
                <img src={opt.src} alt={opt.id} className="w-14 h-14 object-contain" />
              </button>
            ))}
          </div>
        )}
        ...
        {phase === 'icon' ? (
          <button
            ...
            className={
              ...
              (selectedIcon ? 'bg-yellow-300 text-white' : 'bg-gray-200 text-gray-400 cursor-not-allowed')
            }
          >선택 완료</button>
        ) : (
          <button
            ...
            className={
              ...
              (label.trim() ? 'bg-green-400 text-white' : 'bg-gray-200 text-gray-400 cursor-not-allowed')
            }
          >{loading ? '추가 중…' : '추가하기'}</button>
        )}
      </div>
    </div>
  )
